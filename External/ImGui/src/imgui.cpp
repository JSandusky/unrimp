...
static void SettingsHandlerWindow_WriteAll(ImGuiContext* imgui_ctx, ImGuiSettingsHandler* handler, ImGuiTextBuffer* buf)
{
    ...
    for (int i = 0; i != g.SettingsWindows.Size; i++)
    {
        ...
        buf->appendf("Pos=%f,%f\n", settings->Pos.x, settings->Pos.y);		// TODO(co) Changed this from integer to float since "SettingsHandlerWindow_ReadLine()" is reading floats
        buf->appendf("Size=%f,%f\n", settings->Size.x, settings->Size.y);	// TODO(co) Changed this from integer to float since "SettingsHandlerWindow_ReadLine()" is reading floats
